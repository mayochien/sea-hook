// 網站根組件：串起海況地圖／潮汐面板、海釣小遊戲、魚種圖鑑與知識測驗等區塊。
import { useEffect } from "react";
import { CommonFishList } from "./components/CommonFishList";
import { QuizTestForm } from "./components/QuizTestForm";
import { FishingGame } from "./components/FishingGame";
import { TideDetail, TideMap } from "./components/TideSection";
import type { TideMapRegion } from "./components/TideSection";
import { fishQuiz } from "./data/fishReferenceData";
import { fishEntries } from "./data/fishInfo";
import { useSeaConditionStore } from "./store/useSeaConditionStore";
...
const mapRegions: TideMapRegion[] = [
  {
    id: "north",
    label: "北部",
    paths: [
...
function App() {
  ...
  useEffect(() => {
    if (!visibleRegionSet.has(selectedRegion)) {
      ...
      <div className="mx-auto w-full max-w-6xl px-4 pb-12 pt-8 sm:px-6 lg:px-8">
        <header className="mb-8 overflow-hidden rounded-3xl border border-sky-200/20 bg-gradient-to-br from-cyan-500/14 via-sky-400/6 to-slate-900/14 p-6 backdrop-blur-md sm:p-8">
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            <button
              type="button"
              className="rounded-full bg-sky-200/20 px-3 py-1 transition hover:bg-sky-200/30"
              onClick={() => scrollToSection("sea-conditions")}
            >
              海況參考
            </button>
            <button
              type="button"
              className="rounded-full bg-teal-200/20 px-3 py-1 transition hover:bg-teal-200/30"
              onClick={() => scrollToSection("fishing-game")}
            >
              海釣遊戲
            </button>
            <button
              type="button"
              className="rounded-full bg-cyan-200/20 px-3 py-1 transition hover:bg-cyan-200/30"
              onClick={() => scrollToSection("fish-education")}
            >
              魚種教育
            </button>
          </div>
        ...
        <section
          id="sea-conditions"
          className="mb-8 scroll-mt-6 grid gap-4 lg:grid-cols-[1.15fr_1fr] lg:items-stretch"
        >
          <TideMap mapRegions={mapRegions} />
          <TideDetail />
        </section>

        <div id="fishing-game" className="scroll-mt-6">
          <FishingGame fishEntries={fishEntries} />
        </div>

        <section
          id="fish-education"
          className="mb-8 scroll-mt-6 grid gap-4 lg:grid-cols-[1.2fr_1fr]"
        >
          <CommonFishList entries={fishEntries} />
        ...
        <footer className=" text-center text-sm text-cyan-100/80">
          <div className="flex flex-wrap justify-center gap-x-2 gap-y-0.5">
            ...
            <a
              className="transition hover:text-cyan-100 hover:underline"
              href="https://fishdb.sinica.edu.tw/"
              target="_blank"
              rel="noreferrer"
            >
              臺灣魚類資料庫
